...
- **bytes 字段自动解码**：树形响应中会将看起来是 protobuf 的 base64 `bytes` 字段（如 `specData`）自动展开为原始字段结构，同时保留原 base64 值，方便直接查看服务端二次打包的内容。
...
| 日期 | 功能 |
|------|------|
| 2026-08-03 | 响应树自动解码 base64 protobuf `bytes` 字段：无需嵌套 schema 即可展开为原始字段编号结构，原始 base64 仍保留；支持 `specData` 这类字段直接查看 |
